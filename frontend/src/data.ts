...
let points: Points[] = Object.keys(COLORS).map((item) => ({
...
const processData = (data: Points[], zero = false): DisplayData => {
  ...
  const dataNormalizedArray: [string, number, number][] = data.map((item) => [
    item.color,
    item.points / pointsMaxIndex,
    item.points,
  ])
  ...
  const displayData: DisplayData = dataNormalizedArray.map((item, index) => {
    ...
    if (item[2] === previousScore) {
      ...
      if (
        ...
        (item[2] === 0 ||
          ...
          !dataNormalizedArray
            .slice(index)
            .find((searchItem) => searchItem[2] < item[2]))
      ) {
    ...
    const returnObject = {
      color: item[0],
      colorString: item[0].charAt(0).toUpperCase() + item[0].slice(1),
      points: item[2],
      relativePercentage: item[1] * 100,
      currentPercentage: zero
        ? 0
        : dataSubject.value.find((prevItem) => prevItem.color === item[0])
            ?.currentPercentage ?? 0,
      badgeString,
      badgeClass,
    }
